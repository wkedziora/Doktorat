...
crs(frequency) <- crs("+init=epsg:4326")

r_frequency <- raster(frequency, ncol = 100, nrow = 100)
rc_frequency <- rasterize(st_coordinates(sites_gps%>% filter(is.na(tsl))), r_frequency, fun = "count")
tm_shape(rc_frequency) + tm_raster(palette = "Greys", title = "Zagęszczenie") +
        tm_shape(Region, bbox = "Poland", projection="longlat", is.master = TRUE) + tm_borders() +
