...
sites_raw %>%
        ...
        dplyr::mutate(tsl = as.integer(levels(tsl))[tsl],
                      zyzn = case_when(tsl %in% c(11, 12, 13, 14) ~ "B",
                                       ...
                                       tsl %in% c(18) ~ "BWG"),
                      grp = case_when(tsl %in% c(11, 12, 13, 14, 22, 23, 24, 32, 33, 34, 42, 43, 44, 45, 46) ~ "N",
                                      tsl %in% c(55, 56, 61, 62, 63, 64, 65, 66) ~ "W",
                                      tsl %in% c(71, 72, 73, 81, 82, 83, 91, 92, 93, 94, 95, 96) ~ "G",
                                      tsl %in% c(18) ~ "WG"),
                      ...
                      grp = ordered(grp, levels = c("N", "W", "G", "WG")),
