...
server <- c("http://mapserver.bdl.lasy.gov.pl/ArcGIS/services/WMS_BDL/mapserver/WMSServer?")
request <- c("request=GetFeatureInfo&")
service <- c("service=WMS&")
version <- c("version=1.1.1&")
layers <- c("layers=0&") #layer to bbox
styles <- c("styles=&")
old_srs <- c("srs=EPSG%3A4326&")
srs <- c("srs=EPSG:4326&")
bbox <- c("bbox=19.49816426,52.49971120,19.49816427,52.49971121&")
